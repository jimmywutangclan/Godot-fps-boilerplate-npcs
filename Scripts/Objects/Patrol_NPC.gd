...
func Test_Target_Audible(Target):
	if Target.is_in_group("Player"):
		...
		var Current_Player_Speed = Target.move_speed
		
		if Current_Player_Speed == Player_Run_Speed or (Current_Player_Speed > Player_Walk_Speed / 2.0 and Player_Distance <= 5):
